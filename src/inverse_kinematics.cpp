...
Eigen::MatrixXd MakeKKTMatrix(const Eigen::MatrixXd& Jc,
                              const Eigen::MatrixXd& Jt, double reg) {
  if (Jc.cols() != Jt.cols()) {
...
Eigen::VectorXd SolveIKStep(const pinocchio::Model& model, pinocchio::Data& data,
                            const Eigen::VectorXd& q,
                            const std::vector<ConstraintPair>& constraints,
                            const PositionMap& goal_positions,
                            const FrameIndexMap& frame_ids, double reg) {
...
void ConstrainedIKSolver::Solve(const PositionMap& goals, int iterations) {
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    ...
    joint_positions_ = pinocchio::integrate(*pin_model_, joint_positions_, dq_);
  }
}
